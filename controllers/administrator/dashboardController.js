...
export const getAllDevices = async (req, res) => {
    try {
        console.log('getAllDevices: Starting request...');
        ...
        console.log('Sample device data:', devices.length > 0 ? devices[0].toJSON() : 'No devices found');
...
export const createDevice = async (req, res) => {
    try {
        ...
        console.log('Creating new device:', { device_name, device_status, location });
        ...
        console.log(`Device created successfully: ${device.device_name} (ID: ${device.device_id})`);
...
export const updateDevice = async (req, res) => {
    try {
        ...
        console.log(`Updating device ${id} with:`, { device_name, device_status, location });

        const device = await Device.findByPk(id);

        if (!device) {
            console.log(`Device with ID ${id} not found for update`);
            return res.status(404).json({
                status: 'error',
                message: 'Device not found'
            });
...
export const deleteDevice = async (req, res) => {
    try {
        ...
        if (!device) {
            console.log(`Device with ID ${id} not found for deletion`);
            return res.status(404).json({
                status: 'error',
                message: 'Device not found'
            });
...
export const updateDeviceStatus = async (req, res) => {
    try {
        ...
        console.log(`Updating device ${id} status to: ${device_status}`);

        if (!device_status || !['aktif', 'nonaktif'].includes(device_status)) {
            console.log(`Invalid device status provided: ${device_status}`);
        ...
        if (!device) {
            console.log(`Device with ID ${id} not found for status update`);
            return res.status(404).json({
                status: 'error',
                message: 'Device not found'
            });
        ...
        console.log(`Device ${id} status updated to ${device_status} successfully`);
